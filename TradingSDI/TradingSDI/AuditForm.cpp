...
int CAuditForm::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	if (CFormView::OnCreate(lpCreateStruct) == -1)
		...
		desktop.bottom=desktop.bottom-157;
	   ...
	   if (!m_CodeChangGrid.CreateGrid(WS_CHILD|WS_VISIBLE|WS_CLIPCHILDREN|WS_CLIPSIBLINGS,rect2,this,IDC_STATICODECHANGED))
	   {
	   ...
	   if (!m_npbpstype1grid.CreateGrid(WS_CHILD|WS_VISIBLE|WS_CLIPCHILDREN|WS_CLIPSIBLINGS,rect3,this,IDC_STATICNPBPSTYPE1))
	   {
				TRACE0("Failed to create CodeChangedGrid\n");
				return -1; // fail to create
       }
	   ...
	   if (!m_npbpstype2grid.CreateGrid(WS_CHILD|WS_VISIBLE|WS_CLIPCHILDREN|WS_CLIPSIBLINGS,rect4,this,IDC_STATICNPBPSTYPE2))
	   {
				TRACE0("Failed to create CodeChangedGrid\n");
				return -1; // fail to create
       }
