...
#endif
//CUIThread
BEGIN_MESSAGE_MAP(GroupWiseNetPos,CUGCtrl)	
	ON_WM_SIZE()
	ON_WM_TIMER()		
END_MESSAGE_MAP()
int GroupWiseNetPos::insertFilterFlag=0;
GroupWiseNetPos::st_Dealing_Array GroupWiseNetPos::m_st_Dealing_Array;	
GroupWiseNetPos::st_Dealing_Array GroupWiseNetPos::m_st_Dealing_Grid_array;
CMutex GroupWiseNetPos::dealing_mutex;
/////////////////////////////////////////////////////////////////////////////
//Standard MyCug construction/destruction
boolean  Check_numeric_col_filter_2(CString  filter_value,CString  real_value);

UINT Show_GroupwiseNetPos(void *pParam);
GroupWiseNetPos::GroupWiseNetPos()
{
...
void GroupWiseNetPos::OnSetup()
{
	...
	{					
		...
		m_pThreads=AfxBeginThread(Show_GroupwiseNetPos, this);	
	}
	 catch(_com_error & ce)
	{
...
void GroupWiseNetPos::OnSheetSetup(int sheetNumber)
{
	int	nRow = 0, nCol = 0;	
	CUGCell cell;	
	SetGridDefault(&cell);
...
void GroupWiseNetPos::OnTimer(UINT nIDEvent)
{		
	GroupWiseNetPos::dealing_mutex.Lock();	
	...
	{
		if (GroupWiseNetPos::insertFilterFlag==1)
		{
...
if (val_type==0)
	...
	GroupWiseNetPos::dealing_mutex.Unlock();		
...
void GroupWiseNetPos::OnGetCell(int col,long row,CUGCell *cell)
{
...
void GroupWiseNetPos::OnMenuCommand(int col,long row,int section,int item)
{
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
...
void GroupWiseNetPos::filter()
{
	...
	if ( GroupWiseNetPos::insertFilterFlag==0)
	{
			addItemToCombobox();
			GroupWiseNetPos::insertFilterFlag=1;
			InsertRow(0);
	...
	else
	{
		...
		DeleteRow(0);
		GroupWiseNetPos::insertFilterFlag=0;
...
void GroupWiseNetPos::addItemToCombobox()
{
	CStringArray arr;
	CStringArray arr1;
	CStringArray arr2;
...
BOOLEAN  GroupWiseNetPos::CheckvalueInArray(const CStringArray& arr,CString strval) 
{
	try
	{
...
void GroupWiseNetPos::colShorting( int index)
{
	m_str_array.Clear();
...
UINT Show_GroupwiseNetPos(void *pParam)
{
	GroupWiseNetPos* pThis= (GroupWiseNetPos*)pParam;	
	CoInitialize(NULL);
	...
	if(SUCCEEDED(hr))
	{
		...
		{				
			...
			strCommand.Format(L"GroupwiseNetpos'%d'",CMainFrame::int_currenttime);        
			_bstr_t bstrCommand="";
			...
			hr=artists1.Open(session,strCommand_char);							 
			 ...
			 if(SUCCEEDED(hr))
			 {
				 GroupWiseNetPos::m_st_Dealing_Array.Clear();
				 GroupWiseNetPos::st_Dealing m_st_Dealing={};
				 ...
				 {																	  
					...
					GroupWiseNetPos::m_st_Dealing_Array.Add(&m_st_Dealing);
				 }
				 artists1.Close();				    									 			 				 
			 }
...
void GroupWiseNetPos::OnTH_LClicked(int col,long row,int updn,RECT *rect,POINT *point,BOOL processed)
{		
	UNREFERENCED_PARAMETER(row);
