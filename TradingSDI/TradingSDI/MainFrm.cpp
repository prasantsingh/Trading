...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CMFCRibbonEdit* pEditRibbon = DYNAMIC_DOWNCAST(CMFCRibbonEdit, m_wndRibbonBar.FindByID(ID_TOTALVALUE));
	
	pEditRibbon->SetText(L"Total");
...
*/
	...
	//Making Full Size Dockable
	//m_wndOutput.EnableDocking(CBRS_ALIGN_ANY);

	/*m_wndOutput.SetWindowPos(NULL,5 ,500, 100,600 , SWP_NOACTIVATE | SWP_NOZORDER);	
	...
	UINT flags = SWP_NOSENDCHANGING |SWP_NOACTIVATE | SWP_NOZORDER|SW_MAXIMIZE;
	//m_wndOutput.SetWindowPos(NULL, pos.left - 15, pos.top, pos.Width() + 15, pos.Height(), flags);
	//m_wndOutput.SetMinSize(CSize(600,600));

    HDWP hdwp;
    //m_wndOutput.MovePane(CRect(pos.left - 15, pos.top, pos.Width() + 15, pos.Height()),TRUE, hdwp);


	//m_wndOutput.DockToWindow(&m_rateDocking,CBRS_ALIGN_TOP);
	//DockPane(&m_wndOutput, AFX_IDW_DOCKBAR_TOP);
	//DockPane(&m_wndOutput);
	//m_wndOutput.ShowPane(TRUE,FALSE,TRUE);
	//m_wndOutput.DockToFrameWindow(CBRS_ALL,0);
...
//
//
if (!m_date.Create(WS_CHILD|WS_VISIBLE,CRect(340,30,510,20), &m_wndRibbonBar, ID_TOTALVALUE))
{
		TRACE0("Failed to create checkbox\n");
		return -1; // fail to create
}
...
//m_taxt_total
	...
	//Loading symbol list in Array
		...
		HRESULT hr;
		CoInitialize(NULL);		
		hr=connection.OpenFromInitializationString(L"Provider=SQLOLEDB.1;Password=ok@12345;Persist Security Info=True;User ID=sa;Initial Catalog=tradedatabase;Data Source=68.168.101.187;Use Procedure for Prepare=1;Auto Translate=True;Packet Size=4096;Workstation ID=ABHISHEK-P;Use Encryption for Data=False;Tag with column collation when possible=False");			
		if(SUCCEEDED(hr))
		{
