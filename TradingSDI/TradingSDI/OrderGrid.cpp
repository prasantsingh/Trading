...
//		Trace( _T( "Displayed the grid's internal pop-up menu." ) );	
 ...
 void OrderGrid::filter()
 {
	/*if (OrderGrid::insertFilterFlag==0)
	{
			...
			InsertRow(0);
			for (int col_count=0;col_count<14;col_count++)
			{
				CUGCell cell;
				int row=0;
				int col=col_count;
	...
	else
	{
		...
		OrderGrid::strFilter=" and Plugin_Status<>'OKAY' ";
	}
	RedrawAll();*/
 }
...
void OrderGrid::addItemToCombobox()
{
	...
	for (int forcount=0;forcount<14;forcount++)
	{
		...
		if (forcount==0)
		{
			str[forcount]=str[forcount]+L"OKAY"+L"\n";										
		}
	}
...
session.Close();
	...
	if(OrderGrid::m_Client.Connect(L"68.168.101.187",5042)==FALSE)
	{
