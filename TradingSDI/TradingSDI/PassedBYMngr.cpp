...
BEGIN_MESSAGE_MAP(CPassedBYMngr,CUGCtrl)	
	ON_WM_SIZE()
	ON_WM_TIMER()		
END_MESSAGE_MAP()

int CPassedBYMngr::insertFilterFlag=0;
CPassedBYMngr::st_Dealing_Array CPassedBYMngr::m_st_Dealing_Array;	
CPassedBYMngr::st_Dealing_Array CPassedBYMngr::m_st_Dealing_Grid_array;
CMutex CPassedBYMngr::dealing_mutex;
...
void CPassedBYMngr::OnSetup()
{
	...
	{					
		...
		m_pThreads=AfxBeginThread(PassedByMnger, this);	
	}
	 catch(_com_error & ce)
	{
...
void CPassedBYMngr::OnSheetSetup(int sheetNumber)
{
	int	nRow = 0, nCol = 0;	
	CUGCell cell;	
	SetGridDefault(&cell);
...
void CPassedBYMngr::OnTimer(UINT nIDEvent)
{		
	CPassedBYMngr::dealing_mutex.Lock();	
	...
	{
		if (CPassedBYMngr::insertFilterFlag==1)
		{
...
if (val_type==0)
	...
	CPassedBYMngr::dealing_mutex.Unlock();		
...
void CPassedBYMngr::OnGetCell(int col,long row,CUGCell *cell)
{
...
void CPassedBYMngr::OnMenuCommand(int col,long row,int section,int item)
{
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
...
void CPassedBYMngr::filter()
{
	...
	if ( CPassedBYMngr::insertFilterFlag==0)
	{
			addItemToCombobox();
			CPassedBYMngr::insertFilterFlag=1;
			InsertRow(0);
	...
	else
	{
		...
		DeleteRow(0);
		CPassedBYMngr::insertFilterFlag=0;
...
void CPassedBYMngr::addItemToCombobox()
{
	CStringArray arr;
	CStringArray arr1;
	CStringArray arr2;
...
BOOLEAN  CPassedBYMngr::CheckvalueInArray(const CStringArray& arr,CString strval) 
{
	try
	{
...
void CPassedBYMngr::colShorting( int index)
{
	m_str_array.Clear();
...
UINT PassedByMnger(void *pParam)
{
	CPassedBYMngr* pThis= (CPassedBYMngr*)pParam;	
	CoInitialize(NULL);
	CDataSource connection;
	CSession session;
	CCommand<CAccessor<CPassedBYMngrTable> > artists1;	
	HRESULT hr;
	...
	if(SUCCEEDED(hr))
	{
		...
		{				
			...
			char* strCommand_char=(char*)bstrCommand;
			 ...
			 if(SUCCEEDED(hr))
			 {
				 CPassedBYMngr::m_st_Dealing_Array.Clear();
				 CPassedBYMngr::st_Dealing m_st_Dealing={};
				 ...
				 {																	  
					...
					CPassedBYMngr::m_st_Dealing_Array.Add(&m_st_Dealing);
				 }
				 artists1.Close();				    									 			 				 
			 }
...
void CPassedBYMngr::OnTH_LClicked(int col,long row,int updn,RECT *rect,POINT *point,BOOL processed)
{		
	UNREFERENCED_PARAMETER(row);
